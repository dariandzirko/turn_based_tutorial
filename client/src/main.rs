...
fn main() {
    ...
    App::new()
        ...
        .add_system_to_stage(
            ...
            //that can be used to make sure that
            //this system only runs when the client
            //is connected
...
fn setup(mut commands: Commands, asset_server: Res<AssetServer>) {
    ...
    // Spawn a dot in each tile for the hover effect
    for x in 0..3 {
        for y in 0..3 {
            commands
...
fn input(
    ...
    if let Some(mouse_position) = window.cursor_position() {
        ...
        //Note this calculation assume a fixed window size
        //Thats fine for now but consider using window size instead
        ...
        //If left mouse button is pressed, send a place tie event to the server
        if input.just_pressed(MouseButton::Left) {
    ...
    for event in game_events.iter() {
        match event {
            GameEvent::PlaceTile { player_id, at } => {
                let x = at % 3;
                let y = at / 3;
                //Is this texture definition giving me issues
                let texture =
...
//Ie. If client lost connection, internet gone, or server gone
...
fn receive_events_from_server(
    ...
    while let Some(message) = client.receive_message(0) {
        //Whenever the server sends message we know that it must be a game event
